...
function showErrors (errors, form) {
	...
	if (errors.length > 0) {
		...
		for (var errorIndex = 0; errorIndex < errors.length; errorIndex++) {
			...
			var errorContainer = document.getElementById(errors[errorIndex]['id'] + '_error');
			if (errorContainer) {
...
function formval_clearErrors(form) {
	...
	// Inputs
	var arEls = formval_getFormInputsWithErrors(form);
	for (var i = 0; i < arEls.length; i++)
		formval_removeClass(arEls[i], 'error');
	// Error msgs
	var arEls = formval_getFormErrorContainers(form);
	for (var i = 0; i < arEls.length; i++) {
		arEls[i].innerHTML = '';
...
function formval_removeClass (element, theClass) {
	...
	element.className = element.className.replace(classRE, ' ').replace(/  /g, ' ');
}
